...
namespace Habanero.Faces.Base
{
    ...
    public class FlowLayoutManager : LayoutManager
    {
        ...
        private Dictionary<IControlHabanero, Location> locations; 
        private class Location
        {
            public Location(int left, int top, int right)
            {
                Left = left;
                Right = right;
                Top = top;
        ...
        {
            locations = new Dictionary<IControlHabanero, Location>();
            ...
            for (int controlNumber = 0; controlNumber < this._controls.Count; controlNumber++)
            {
                ...
                Location location = new Location(ctl.Left, ctl.Top, ctl.Right);
                locations.Add(ctl, location);
            }
            ...
            for (int controlNumber = 0; controlNumber < this._controls.Count; controlNumber++)
            {
                ...
                if (ctl.Visible)
                {
                    ...
                    SetControlPosition(ctl, locations[ctl]);
            ...
            {
                IControlHabanero ctl = GetControl(controlNumber);
                Location location = locations[ctl];
                if (ctl.Left != location.Left) ctl.Left = location.Left;
                if (ctl.Top != location.Top) ctl.Top = location.Top;
            }
        }
        ...
        private void SetControlPosition(IControlHabanero ctl, Location newLocation)
        {
            int newLeft;
            ...
            int newTop = _currentPos.Y;
            newLocation.Top = newTop;
            //if (ctl.Top != newTop) ctl.Top = newTop;
        }
        ...
        private void ShiftControlsRightForCentering(int startControlNum, int endControlNum)
        {
            ...
            {
                Location location = locations[_controls[ctlNum]];
                int lastLocationInRow = locations[_controls[endControlNum]].Right;
                location.Left += (ManagedControl.Width - lastLocationInRow - BorderSize)/2;
            }
        }
