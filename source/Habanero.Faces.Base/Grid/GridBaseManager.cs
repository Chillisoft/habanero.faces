...
namespace Habanero.Faces.Base
{
    ...
    public class GridBaseManager
    {
        ...
        private IBindingListView _dataTableDefaultView;
        /// <summary>
        ...
        protected IBindingListView GetBindingListView(IBusinessObjectCollection boCol)
        {
            ...
            IBindingListView dataView = DataSetProvider.GetDataView(uiDef.UIGrid);
            _dataTableDefaultView = dataView;
...
/*                foreach (DataRowView dataRowView in _dataTableDefaultView)
        ...
        private DataRowView GetDataRowView(int rowIndex)
        {
            try
            {
                return (DataRowView)_dataTableDefaultView[rowIndex];
            }
        ...
        {
            if (_dataTableDefaultView == null && this._gridBase.DataSource != null)
            {
                _dataTableDefaultView = this._gridBase.DataSource as IBindingListView;
            }
            if (_dataTableDefaultView == null)
           {
                
                throw new GridBaseInitialiseException
                    ("You cannot apply filters as the collection for the grid has not been set with a IBindingListView");
            }
...
/*                //Temp need to support backward compatibility of the search criteria
                if (!string.IsNullOrEmpty(this._gridBase.AdditionalSearchCriteria))
                {
                    if (!string.IsNullOrEmpty(searchClause))
                    {
                        searchClause += " AND ";
                    }
                    searchClause += GridControl.AdditionalSearchCriteria;
                }*/

                _dataTableDefaultView.Filter = filterClauseString;
