...
namespace Habanero.Faces.Test.Base.Mappers
{
	...
	class ControlMapperStub2 : ControlMapper
	{
		public ControlMapperStub2(IControlHabanero ctl)
			: base(ctl, RandomValueGen.GetRandomString(), false, MockRepository.GenerateStub <IControlFactory>())
		{
		}
	...
	public abstract class TestControlMapper 
	{
		...
		public void TestCreateMapperWithAssembly()
		{
			...
			IControlMapper mapper = ControlMapper.Create
				("Habanero.Faces.Base.TextBoxMapper", "Habanero.Faces.Base", b, "Test", false, GetControlFactory());
			Assert.AreSame(typeof (TextBoxMapper), mapper.GetType());
			Assert.AreSame(b, mapper.Control);
		}
...
//        [Test, Ignore("Need to move tests to include BO lookups and then refactor.")]
	...
	public class MyTestBO : BusinessObject
	{
		...
		protected MyTestBO(SerializationInfo info, StreamingContext context)
			: base(info, context)
		{
		}
